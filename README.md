...
Um auf Windows Python 3.13 zu installieren, geben Sie in der Windows Eingabeaufforderung den Befehl `winget install Python.Python.3.13` ein und drücken Enter. Python wird installiert.
...
Öffnen Sie den entpackten Projektordner `IU_Dashboard` im Explorer.
...
Um die Umgebung zu aktivieren, geben Sie `.venv\Scripts\activate.bat` ein.
...
Nach Erfolgreicher Installation lässt sich das Dashboard mit dem Befehl
`py -m src.app` starten.
